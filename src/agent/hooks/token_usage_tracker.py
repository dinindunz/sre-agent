...
class TokenUsageTracker(HookProvider):
    ...
    def __init__(self):
        ...
        self.pricing = get_config().model_pricing
    ...
    def _log_cycle_details(
        ...
        """Log detailed breakdown for a cycle."""
        logger.info(f"[TokenUsage] [Cycle {cycle_num}]")
        logger.info(
            f"[TokenUsage]   Input: (tokens: {input_tokens}, cost: ${costs['input']:.6f}), "
            f"Output: (tokens: {output_tokens}, cost: ${costs['output']:.6f})"
        )

        if cache_read > 0 or cache_write > 0:
            logger.info(
                f"[TokenUsage]   CacheRead: (tokens: {cache_read}, cost: ${costs['cache_read']:.6f}), "
                f"CacheWrite: (tokens: {cache_write}, cost: ${costs['cache_write']:.6f})"
            )
            logger.info(
                f"[TokenUsage]   FinalCost: ${total_cost:.6f} "
                f"(input_cost + output_cost + cache_read_cost + cache_write_cost)"
            )
            if savings != 0:
                logger.info(
                    f"[TokenUsage]   NetSavings: ${savings:.6f} "
                    f"(cache_read_benefit - cache_write_penalty)"
                )
        else:
            logger.info(f"[TokenUsage]   FinalCost: ${total_cost:.6f} (input_cost + output_cost)")
    ...
    def track_token_usage(self, event: BeforeModelCallEvent) -> None:
        ...
        if display_cycle > 0:
            ...
            cycle_savings = self._calculate_cache_savings(cycle_cache_read, cycle_cache_write)
    ...
    def track_final_usage(self, event: AfterInvocationEvent) -> None:
        ...
        logger.info(
            f"[TokenUsage] === Final Summary === "
            f"Input: {final_usage['inputTokens']}, Output: {final_usage['outputTokens']}, "
            f"CacheRead: {final_usage.get('cacheReadInputTokens', 0)}, "
            f"CacheWrite: {final_usage.get('cacheWriteInputTokens', 0)}, "
            f"TotalCost: ${total_cost:.6f}"
            + (f", NetSavings: ${total_savings:.6f}" if total_savings != 0 else "")
        )
